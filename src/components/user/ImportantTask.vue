...
<script>
  export default {
    ...
    methods: {
      ...
      getImportantTodos(){
        let isInstore = this.$store.getters.getImportanttodos.length;
        console.log(isInstore)
        if(isInstore != 0){
          this.tableData = this.$store.getters.getImportanttodos;
          this.loading = false;
          console.log('直接从仓库取得');
        }else{
          this.axios.get('importanttodos.php').then( response => {
              this.loading = true;
              let res = response.data;   //用res承接返回后台的json文件(像使用数组那样)
              console.log(res[0])
              if(res[0].status == 1){
                this.$store.commit('setImportantTodos', res);
                this.tableData = this.$store.getters.getImportanttodos;
                this.loading = false;
                console.log('通过api取得');
