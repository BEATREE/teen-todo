<template>
    <el-form :model="addTodoForm" :rules="rules" ref="addTodoForm" label-width="80px">
        ...
        <el-form-item :label-width="formLabelWidth">
            <el-button type="primary" icon="el-icon-upload" @click="addToDo()">添加ToDo</el-button>
        </el-form-item>
    ...
    export default {
        ...
        methods:{
            addToDo() {
                ...
                this.axios.post('addtodos.php', this.addTodoForm).then( response => {
                    let res = response.data;
                    if(res.status === 1){
                        this.addTodoForm.tid = res.tid;
                        this.addTodoForm.timportant = Boolean(parseInt(this.addTodoForm.timportant));
                        if(this.$store.getters.getAlltodos.length != 0){
                            this.$store.commit('addTodos', this.addTodoForm);
                            this.$store.commit('clearTodayTodos');
                            this.$store.commit('clearImportantTodos');
                        }else{
